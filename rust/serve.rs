...
mod tests {
    ...
    fn web_rows_child_process() {
        ...
        // What is left to sweep out of bytes this module writes: every
        // read page that listed rows here - `/decided`, `/boards`,
        // `/board/{project}`, `/lanes`, `/sprints`, `/sprints/{project}`,
        // `/sprint/{project}/{id}`, `/plans`, `/deployments`,
        // `/deployment/{project}/{id}` and `/subscriptions` - is the
        // mounted application's since `t-bf255880` wave 1 and answers the
        // shell, so their rows are swept in the browser by
        // `read_pages_are_rows_with_one_pill_and_a_mono_priority_in_real_chrome`.
        // Listing a mounted route here would measure the mount point and
        // prove nothing, so the three below are the whole server-rendered
        // set.
        let routes = ["/all", "/search?q=render", task_route.as_str()];
        let mut measured = 0;
        for route in routes {
        ...
        // The floor is an anti-vacuity guard rather than a count: `/all`
        // and `/search` list rows only when the fixture has them, and this
        // one seeds the epic the task detail lists. It still has to be
        // measuring something, because a sweep over three routes that
        // found no row at all would pass while proving nothing.
        assert!(
            measured >= 1,
            "the sweep measured no rows at all, so it proved nothing"
        );
    ...
    fn web_counts_child_process() {
        ...
        for route in ["/all", "/search", "/no/such/page"] {
            let html = render(route).unwrap_or_else(|error| panic!("render {route}: {error}"));
