...
/**
 * How the queue is on screen: one card at a time, or all of it at once.
 *
 * Two layouts, ONE component, because they are one surface: the same queue,
 * the same cards, the same four POSTs, the same receipts and the same undo.
 * `/all` was a second renderer of the deck's markup on the server
 * (`all_open` over `decision_card`) and `t-bf255880` deletes that renderer
 * rather than porting it: a list that answered differently from the deck
 * would be the drift ADR-048 §5 forbids, wearing a different address.
 *
 * What the layout decides is presentation and nothing else:
 *
 * - `deck` shows the card the reader is on and hides the rest, advances on
 *   an answer, animates the swap and keeps focus on the current card.
 * - `list` shows every open card in the projection's order, advances
 *   nothing, animates nothing and takes focus from nobody — a reader
 *   scrolling a list is already where they want to be.
 */
export type DeckLayout = "deck" | "list";
/**
...
export function Deck({ layout = "deck" }: { layout?: DeckLayout } = {}): ReactElement {
   ...
   *
   * The list advances nothing and takes focus from nobody: every card is
   * already on screen, so an animation would be one card of a scrolling
   * page flashing for no reason, and a focus move would scroll the reader
   ...
   *
   * There is nothing to skip past on the list: the next card is already
   * under this one, and reordering the page under a reader's thumb would
   * lose their place.
   */
  const skip = useCallback(() => {
    if (list || current === null || queue.length < 2) {
      return;
    }
    leave(current, "forward");
    setOrder((previous) => [...previous.filter((id) => id !== current), current]);
  }, [current, leave, list, queue.length]);

  /** `ArrowLeft` walks back through the queue, recording nothing. */
  const back = useCallback(() => {
    if (list || queue.length < 2 || index === 0 || current === null) {
      return;
    }
    leave(current, "back");
    setPosition(index - 1);
  }, [current, index, leave, list, queue.length]);
  ...
  const onKey = useCallback(
    (event: KeyboardEvent) => {
      ...
      // mean. On the LIST there is no such card: every open item is on
      // screen, so a digit is only a decision on the card the reader has
      // actually put focus on.
      const acting =
        cardNode ?? (list || current === null ? null : nodes.current[current]);
  ...
  /** How many boards the open queue is spread over, as the list says it. */
  const boards = new Set(
    rendered.map((id) => byId.get(id)?.board).filter((board) => board !== undefined),
  ).size;
  ...
  const stateOf = (id: string): CardState => {
    ...
    return {
      // Every card of the list is on screen and none of them is "the one
      // the reader is on": the deck's single-card state has no meaning
      // where the reader can see the whole queue.
      current: !list && id === current,
      hidden: !list && id !== current && (ghost === null || ghost.id !== id),
      sending: open ?? null,
      motion:
  ...
  return (
    <>
      ...
      <main
        id="main"
        {...(list
          ? { "data-page": true, "data-list": true, "data-route": "all" }
          : { "data-deck": true, "data-deck-position": index })}
      ...
      >
        ...
        {list ? (
          <p className="explain">
            Each card is one question an agent is waiting on. Pick an answer and it is
            recorded on the board at once; the agent continues from there. Undo any
            decision from Recent decisions.
          </p>
        ) : null}
        {list ? (
          queue.length === 0 ? null : (
            <p className="count" data-testid="list-count">
              <span data-open-count>{queue.length}</span> open across {boards}{" "}
