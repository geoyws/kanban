...
/* SCRATCH: writer A's router switch, to the published contract. Take A's
   app.tsx at the merge; the four lines below are writer C's cases. */
function renderPage(route: Route): ReactElement | null {
  switch (route.name) {
    case "sprints":
      return <SprintsPage route={route} />;
    case "board-sprints":
      return <BoardSprintsPage route={route} />;
    case "sprint":
      return <SprintPage route={route} />;
    case "plans":
      return <PlansPage route={route} />;
    default:
      return null;
  }
}
...
export function App(): ReactElement {
  ...
  return (
    <>
      ...
      {route.name === "needs-you" ? null : renderPage(route)}
      {route.name !== "needs-you" ? null : (
        <main
          id="main"
          data-deck
