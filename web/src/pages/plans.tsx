...
/** `rust/serve.rs`'s `priority_badge`: the one value in a row that is not prose. */
function Priority({ task }: { task: PlanTask }): ReactElement {
  ...
  return (
    <span
      ...
      title={`stored priority ${task.priority}`}
    >
      {level}
    </span>
  );
  ...
  return (
    <article className="plan" data-plan={plan.id} data-testid="plan">
      ...
      <p className="meta">
        ...
        as <code>{plan.id}</code> at <Priority task={plan} />
            ...
            {card.children.map((child) => (
              <li key={child.task.id}>
                ...
                <p className="meta">
                  ...
                  at <Priority task={child.task} />, filed as{" "}
                  <code>{child.task.id}</code>
