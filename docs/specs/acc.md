...
- 2026-09-25 — a removed task's sitrep, handoff, deployment and attention
  ...
  `rust/db.rs`), so every read path gates the orphaned id on the task's
  last-known removal tags — `task_linked_row_visible` in the listings,
  counts and lanes, `document_row_tags` in search, `retire_handoff` and
  `accept_handoff`, and the deployment subject gates — failing closed with
  no removal record, exactly as `subscriptions.subject_task_id` already
  does. A row naming no task keeps board scope: session handoffs, lanewide
  sitreps and taskless deployments are unchanged, and rows already nulled
  by an older removal stay NULL. Mutations that touch an orphaned row still
  audit: the event's `_semanticV1` snapshot freezes the
  same removal union (failing closed with no removal record) instead of
  failing the write against the gone live row. Requirement wording is
  unchanged: this restores the authorize-as-the-containing-row rule for the
  removal case the `ON DELETE SET NULL` links had silently left.
...
### A25 — a removed task's linked rows stay tag-gated on every surface (`ACC-14`)
...
listing, in search, on served `/api/v1/lanes` or in the deployment listing,
and `handoff retire`, `handoff accept`, `attention show`, `deploy show` and
`deploy finish` answer exactly as for a never-created id — the same exit code
and byte-identical stderr carrying the generic denial, with nothing recorded
— while the owner still sees every row. *When* no enforcement applies, *then*
an unknown id keeps its plain `not found` message. The board schema moves
`33` → `34`: the four links become plain `TEXT` with no foreign key, so a
removal keeps the orphaned id and every read path gates it on the task's
last-known removal tags. Rows already nulled by an older removal recover
their link from their creation event — but only when that event's task has a
...
### A26 — a removed task's id is never reused (`ACC-14`)
...
*Given* a managed caller holding board read, the `visible` tag and board
read on a second board but no `secret` scope, *when* the two newest events
...
of more than 1` truncation notice, `deploy list --status started --limit 1`
and `--status failed --limit 1` each return the newest visible attempt,
served `/api/v1/task/Alpha/t-visible` returns fifty events with
`truncated: true` led by the newest visible one, and served
`/api/v1/deployments` returns a hundred active and thirty failures each
with `truncated: true` — none of them naming the hidden rows — while the
owner still reads the denied rows first on every surface, and granting
...
cursor walks the next page instead of re-reading the same rows. *When* a
delivery fills mid-page on a short final page, *then* the tail reports
unexhausted until the cut consumes the page's last raw row, so the websocket
notices rescan on the next tick instead of parking. Authorization is at scan
...
| `ACC-14` | MUST | http + process | `a_tag_denied_checked_row_shows_no_check_metadata_and_answers_no_check_post_over_http`; `denied_and_unknown_ids_answer_identically_on_every_by_id_attention_surface`; `search_scores_are_a_function_of_permitted_documents_only`; `search_scores_and_order_are_a_function_of_permitted_documents_only`; `denied_and_unknown_task_ids_answer_identically_on_task_routes`; `store::tests::managed_notes_checkpoints_and_named_claim_deny_denied_and_unknown_tasks_identically`; `a_removed_tasks_trail_stays_tag_gated_on_every_tail_over_http`; `removed_task_ids_are_never_reused_and_probe_like_live_denied_ids`; `reusing_a_task_id_is_refused_with_a_plain_message_where_no_guard_can_deny`; `task_attach_writes_refuse_a_tag_denied_task_like_an_unknown_id`; `task_linked_rows_withhold_a_tag_denied_task_on_every_listing`; `attention_by_id_withholds_rows_on_a_tag_denied_task`; `subscription_relation_targets_withhold_a_tag_denied_task_on_read`; `store::tests::removed_task_tag_union_fails_closed_when_a_snapshot_names_no_tags_array`; `residual_lease_sprint_and_deployment_ids_answer_identically_under_enforcement`; `task_removal_keeps_linked_rows_tag_gated`; `compiled_binary_accepts_a_handoff_on_a_removed_task_and_archives_it`; `compiled_binary_hides_an_orphan_deployment_and_doctor_reports_it`; `schema_34_backfills_pre_v34_nulled_links_from_creation_events`; `store::tests::managed_deployment_listing_hides_an_orphan_link_and_doctor_reports_it`; `import_requires_whole_board_write_and_names_no_denied_id`; `compiled_binary_doctor_reports_a_nulled_row_from_a_reused_live_task_id`; `managed_pages_fill_past_denied_rows_with_a_true_truncation_probe`; `dependency_replacement_keeps_a_tag_denied_prerequisite`; `watch::tests::a_limit_1_follow_poll_advances_by_the_scan_floor_over_denied_rows`; `watch::tests::a_one_shot_watch_behind_denied_rows_reports_progress_not_silence`; `store::tests::scan_event_tail_reports_unexhausted_when_delivery_fills_mid_short_page` | tag-denied checked row shows no id or check metadata on any HTTP read; the check POST answers the existing non-enumerating denial byte-identically for denied and unknown ids and records nothing; every by-id surface (web check/reply/reopen POSTs; CLI attention show/resolve/reopen/check and task show) answers a denied id and a never-created id byte-identically under a managed principal, past-guard refusals keep their sentences, and unmanaged boards keep plain not-found messages; a tag-denied document moves no permitted hit's served `lexicalScore`, `score` or order (A21); every task route that names a row (task move/update/remove, claim, events --task, deploy show) refuses a denied and a never-created task id with identical stderr and exit code, and a task-filtered listing succeeds for both alike with byte-identical empty stdout; a sitrep, handoff, subscription or attention row naming a task is listed only for a caller who can read that task, with the unfiltered listings, the attention and handoff counts, served `/api/v1/lanes`, `handoff retire`/`accept` and `subscription show`/`pause`/`resume` obeying the same rule; an untagged attention row hung on a denied task answers every by-id surface like an unknown id, a subscription whose subject or relation target the caller cannot read is withheld on list, show, pause and resume, and a removal snapshot with no tags array fails closed with the stale-entry tag; notes, checkpoints and a named claim carry the same denial at store level, and heartbeat, release, sprint plan candidates and parent epics, deploy finish, abandon and retry-of, and removed-task watch and subscription subjects refuse a denied and a never-created id byte-identically with nothing recorded (A22); a removed task's events stay tag-gated on board-wide `events`, `watch` and served search for readers without the tag, and stay readable to a holder of it, while `events --task` on a removed row is refused for every caller because the row is gone (A23); every write that attaches a row to a task refuses a tag-denied task exactly like a never-created one and records nothing (A24); a removed task's sitrep, handoff, attention and deployment rows keep the orphaned id and stay gated on the removal union on every listing, search, lanes and by-id surface (A25); an orphaned handoff accepts as a lease-free acknowledgement and archives once old, an orphaned deployment hides its one row while `doctor` reports it, and pre-V34 nulled rows recover their link from their creation event (A25); a removed task id is never reused through task add or import: under enforcement a re-add probes exactly like a live denied id with nothing recorded, and where no guard can deny the refusals are the plain already-exists and was-removed messages; an import, its dry-run preview and its `--verify` comparison run only for a principal holding the whole board, and every other authority is refused the generic denial naming no id with nothing written; a NULL-linked row whose creation event names a removed-but-live-again task id is reported for the owner under `reusedTaskLinks` instead of being certified (A26); paged event and deployment listings fill past denied rows with a full visible page and a true `+1` truncation probe on the CLI and the served task-detail and `/deployments` routes, with the owner's newest-first order unchanged, paging by keyset inside one snapshot per page while the cursor-driven tails advance past denied rows a bounded scan at a time (`store::tests::managed_ascending_tail_walks_a_denied_stretch_in_bounded_scans`) (A28); a dependency-list replacement keeps the prerequisite edges the caller cannot read, so the caller's listing still shows only the edges it named while the gate and the owner keep every edge (A27) |
...
- 2026-09-25 — ACC-14 removed-task-link evidence landed (`t-2cffbe08`):
  `task_removal_keeps_linked_rows_tag_gated` seeds a sitrep, a handoff, an
  untagged attention row and a deployment attempt on a `secret` task and then
  removes it: a caller holding only board read and write sees no trace of
  the rows in any listing, in search, on served `/api/v1/lanes` or in the
  deployment listing, and `handoff retire`, `handoff accept`, `attention
  show`, `deploy show` and `deploy finish` answer byte-identically to a
  never-created id — while the owner still sees every row (A25). The landing
  fixed the leak as a bug; requirement wording is unchanged. The four links
  were `ON DELETE SET NULL`, so removing the task rewrote each row's
  `task_id` to NULL and every gate read the orphaned row as board scope.
  The board schema moves `33` → `34`: the links are plain `TEXT` with no
  foreign key — the `subscriptions.subject_task_id` precedent — so a removal
  keeps the id and the listings, the index, the handoff transitions and the
...
- 2026-09-25 — ACC-14 id-reuse evidence landed (`t-2cffbe08`):
  ...
  listing, and the owner still reads it (A26).
  ...
  a removed id answers `task X was removed and its id cannot be reused`.
  The landing fixed the leak as a bug and added the A26 wording: `task add`
  ...
  surviving linked row under the new row's tags. `add_task_in_sprint` now
  ...
  id's tags gets the generic denial — and import refuses the same ids
  instead of upserting over them. No existing test or workflow re-added a
  removed id.

- 2026-09-25 — ACC-14 orphan follow-ups landed (`t-2cffbe08`):
  accepting a handoff orphaned by removal is an acknowledgement that mints no
  lease, and the archive sweep files orphaned handoffs and events away once
  they are old; a deployment naming a task with neither a live row nor a
  removal record hides just that row, and `doctor` reports it under
  ...
  and `schema_34_backfills_pre_v34_nulled_links_from_creation_events` (A25).

- 2026-09-25 — ACC-14 import gate and reuse-report evidence landed: an import
  rewrites arbitrary rows, deletes claims and dependencies with `--reconcile`
  and seizes live leases with `--force`, so `normalize_and_insert` and the
...
- 2026-09-25 — ACC-14 page-fill evidence landed (`t-b694bc83`):
  ...
  from `events --task`, board-wide `events` and `deploy list` on the CLI
  with a true `+1` truncation probe, and from served `/api/v1/task` and
  `/api/v1/deployments` with observed `truncated` flags — while the owner
  still reads the denied rows first and granting `secret` restores them
  (A28). The landing fixed the leak as a bug; requirement wording is
...
- 2026-09-25 — ACC-14 page-fill follow-up (`t-b694bc83` review): the chunked
  ...
  cursor-driven tails (`events_since_filtered`, the websocket notices, the
  watch poll) cap the raw rows examined per scan and advance the cursor past
...
- 2026-09-25 — ACC-14 tail-liveness follow-up (`t-b694bc83` review): the watch
  ...
  empty poll at the head stays silent; `exhausted` now holds only when the
  delivered cut consumed the page's last raw row, so a delivery that fills
  mid-page on a short final page still reschedules the websocket notices.
  Pinned by
  `watch::tests::a_limit_1_follow_poll_advances_by_the_scan_floor_over_denied_rows`,
  `watch::tests::a_one_shot_watch_behind_denied_rows_reports_progress_not_silence`
  and `store::tests::scan_event_tail_reports_unexhausted_when_delivery_fills_mid_short_page`
  (A28).
