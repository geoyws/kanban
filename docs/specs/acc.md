...
- 2026-09-25 — ACC-14 tail-liveness follow-up (`t-b694bc83` review): the watch
  ...
  walking; `exhausted` now holds only when the delivered cut consumed the
  page's last raw row, so a delivery that fills mid-page on a short final
  page still reschedules the websocket notices. Pinned by
