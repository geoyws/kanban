...
| Trusted edge actor header | Separate compiled processes prove `kanban serve --actor-header NAME` rejects missing, duplicate, spoofed, malformed and oversized actor headers, keeps same-origin required, and records a distinct trusted SSO actor as both the durable `resolvedBy` value and the event actor when it settles an attention item raised by a different principal. CLI `attention resolve` still rejects a non-geo, non-raiser actor. |
